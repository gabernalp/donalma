...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.types.update", [$type->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="image">{{ trans('cruds.type.fields.image') }}</label>
            ...
            <div class="form-group">
                ...
                @if($errors->has('globals'))
                    <span class="text-danger">{{ $errors->first('globals') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $type->description) }}">
...
Dropzone.options.imageDropzone = {
    url: '{{ route('admin.types.storeMedia') }}',
    maxFilesize: 5, // MB
...
@if(isset($type) && $type->image)
      var files = {!! json_encode($type->image) !!}
          for (var i in files) {
          var file = files[i]
